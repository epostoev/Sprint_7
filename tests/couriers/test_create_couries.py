...
class TestCreareCourier:
    ...
    def test_create_courier_with_existing_login_conflict(self, courier_methods, delete_courier):
        courier_data, status_code, params = courier_methods.create_courier(Courier.COURIER_DATA)
        print(f"\nPARAMS = {params}")
        print(f"STATUS_CODE = {status_code}")
        print(f"COURIER_DATA = {courier_data}")
        with allure.step("Повторно отпавить запрос на создание курьера с тем же логином"):
            courier_data, status_code, _  = courier_methods.create_courier(Courier.COURIER_DATA)
        with allure.step('Проверить, что статус-код 409 и правильное сообщение об ошибке'):
            assert (status_code == 409 and "Этот логин уже используется. Попробуйте другой." == courier_data.get("message"))
        delete_courier.append((params['login'], params['password'])) 

    @pytest.mark.parametrize('field', ["login", "password"])
    @allure.title('Нельзя создать курьера без обязательных полей LOGIN или PASSWORD')
    def test_create_courier_without_required_field_error(self, courier_methods, field, delete_courier):
        ...
        with allure.step('Проверить, что статус-код 400 и правильное сообщение об ошибке'):
            assert (status_code == 400 and "Недостаточно данных для создания учетной записи" == courier_data.get("message"))
